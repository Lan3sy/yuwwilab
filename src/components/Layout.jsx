            ...
            return (
              <button
                ...
                <span style={{
                  ...
                  <t.Icon size={22} color={active ? '#111' : '#bbb'} />
                </span>
                {t.label}
              </button>
            )
      ...
      {dialog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)',
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: 28,
            width: '100%', maxWidth: 340, boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
            <div style={{ fontSize: 40, textAlign: 'center', marginBottom: 12 }}>⚠️</div>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: '#111', marginBottom: 8, textAlign: 'center' }}>
            ...
            <button onClick={() => { unsavedChanges.current = false; navigate(dialog.to); setDialog(null) }}
              style={{ width: '100%', padding: 14, background: '#fff', border: '1.5px solid #ff4444',
                borderRadius: 12, color: '#ff4444', fontWeight: 700, fontSize: 14,
                marginBottom: 8, cursor: 'pointer' }}>
              Уйти без сохранения
            </button>
            <button onClick={() => setDialog(null)}
              style={{ width: '100%', padding: 14, background: '#111', border: 'none',
